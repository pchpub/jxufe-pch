...
function showBannerAttentionTips(newmenucode, newTitle){
	...
	/*
	var attention = "<img src='../frame14/img/nav/appraise.png' title='&#20851;&#27880;' style='border:0px;width:20px;height:20px;'></img>"; //汉字[+关注]的unicode码
	var appraise = "<img src='../frame14/img/nav/focus.png' title='\u8bc4\u4ef7' style='border:0px;width:20px;height:20px'></img>"; //汉字[+评价]的unicode码
...
NormalUseMenuBeanList.prototype.saveToDB = function(pageUrl,oldbannerHtml,menuname,menucode){
	   //alert("pageUrl="+pageUrl+"......"+base64.encode64(pageUrl));
	   //alert("oldbannerHtml="+oldbannerHtml+"......"+base64.encode64(oldbannerHtml));
...
NormalUseMenuBeanList.prototype.displayOnPage0 = function(){
     var result_href = [''];
     ...
     if (this.normalMenuList.length>0){
      ...
      for (var i=0;i<length;i++){
          ...
          result_href.push('<span style="color:#EC9C14;font-size:13px;">&diams;</span><a href="javascript:void(0)" style="text-decoration : none;color:black;margin-top:12px;padding-left:2px;" ');
          result_href.push('');
...
NormalUseMenuBeanList.prototype.displayOnPage = function(){
     ...
     if (this.normalMenuList.length>0){
      ...
      for (var i=0;i<length;i++){
          ...
          result_href.push('<span style="color:#EC9C14;font-size:13px;">&diams;</span><a href="javascript:void(0)" style="text-decoration : none;color:black;margin-top:12px;padding-left:0px;" ');
          result_href.push('');
          result_href.push(' title=\''+menuname+'\'');
          result_href.push(' onclick="NormalPageOpenByMenucode(\''+menucode+'\')" menucode=\''+menucode+'\'> ');
          if (menuname.length>12)
...
function NormalPageOpenByMenucode(menucode){
  ...
  for (var i=0;i<kingo_list_normal_use_menuitem.normalMenuList.length;i++){
       ...
       if (temp.getMenucode()==menucode){//如果里面有了
           ...
           menuname = temp.getMenuname();
           break;
        }
  }
  var banner_direction = getBanner_direction_obj();
  // 显示当前操作提示信息
  banner_direction.innerHTML = oldbannerHtml;
  // 显示加关注信息
  showBannerAttentionTips(menucode, menuname);
  // 显示常用功能页面
  var frmDesk = document.getElementById("frmDesk").contentWindow.document ;
  if(menucode=="TF" || menucode=="Z2" || pageUrl.startWith("http") || pageUrl.startWith("https")){
  	  window.open(pageUrl);
  }
  else{
